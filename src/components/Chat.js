...
const Chat = ({ send, set, message, messagesList }) => {
  ...
  return (
    ...
    border="1px solid rgba(255, 255, 255, 0.1)">
      ...
      <InputGroup boxShadow="0px 8px 32px rgba(31, 38, 135, 0.37)" overflow={'hidden'} borderRadius='lg'>
        <Input color='whiteAlpha.900' bg="rgba(255, 255, 255, 0.1)" value={message} border={'InactiveBorder'} onChange={(event) => set(event.target.value)} onKeyDown={handleKeyPress} />
        <InputRightAddon p='0px' >
          <Button borderRadius={'none'} variant={'ghost'} colorScheme='whiteAlpha.900' onClick={() => send()}>Send</Button>
        </InputRightAddon>
      </InputGroup>
    </Flex>
  )
